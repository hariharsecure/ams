...
SCHEMA_VERSION = "ams.ams_codex.shareability_receiver_trial.v0"
...
def _run_python_compile(root: Path) -> dict[str, Any]:
    if not (root / "ams_codex").exists():
        return _skipped_command("python_compile")
    return _command_summary(
        _run_command(root, [sys.executable, "-m", "compileall", "-q", "ams_codex"], timeout_seconds=180)
    )
