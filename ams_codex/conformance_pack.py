...
SCHEMA_VERSION = "ams.ams_codex.conformance_pack.v0"
...
def _python_compile(root: Path, *, timeout_seconds: int) -> dict[str, Any]:
    files = [str(path.relative_to(root)) for path in sorted((root / "ams_codex").glob("*.py"))]
...
def _store_check(path: Path) -> dict[str, Any]:
    ...
    replay_check = importlib.import_module("ams_codex.replay").replay_check
    run_replay_oracle = importlib.import_module("ams_codex.replay_oracle").run_replay_oracle
