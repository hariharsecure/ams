...
SCHEMA_VERSION = "ams.ams.work_mode_decision.v0"
    ...
    if decision in {"cleanup_audit", "both"}:
        commands.extend(
            [
                ...
                _command(
                    mode="cleanup_audit",
                    command=f"python3 -m ams.cli --store {audit_store_path} architecture-audit --source-root {source_root} --package-name ams",
