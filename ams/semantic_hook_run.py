...
SCHEMA_VERSION = "ams.ams.semantic_hook_run.v0"
...
def validate_semantic_hook_record_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    if record.get("schema_version") != "ams.ams.semantic_hook_record.v0":
...
def _semantic_hook_records(
    *,
    ...
    for family in required_records:
        ...
        record = {
            "schema_version": "ams.ams.semantic_hook_record.v0",
    ...
    for hook in required_hooks:
        entry: dict[str, Any] = {
            "hooks": [
                {
                    "type": "command",
                    "command": "python3 -m ams.cli semantic-hook-run --semantic-oracle-review-id <review-id>",
